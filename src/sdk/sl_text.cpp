...
Ui::Text::Text(Display *display, const char *text, int x, int y, int width, int height, DrawableObject *parent, int alignment, const char *fontName, bool transparent, bool border):
    Ui::DrawableObject(display, x, y, width, height, parent),
    _text(text),
    _transparent(transparent),
    _fontName(fontName),
    ...
    if (_visible && !_text.empty()) {
        ...
        if (font) {
            ...
            if (isAlignedTo(Alignment::Top))
                ...
                textY = _y + _height - ((_height - textHeight) >> 1) - 2;
